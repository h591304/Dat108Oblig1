...
public class HamburgerBrett {
	...
	public synchronized void leggTilHamburger() throws InterruptedException {
		...
		while(true) {
			...
			burgerKo.add(burger);
			System.out.println(Thread.currentThread().getName() + " legger på hamburger. Brett: " + "(" + burger + "). Brett: " + burgerKo);
			burger++;
	...
	public synchronized void taAvHamburger() throws InterruptedException {
		...
		while(true) {
			...
			while(burgerKo.size() == 0) {
				System.out.println(Thread.currentThread().getName() + " ønsker å ta av hamburger, men brett tomt. Venter!");
				wait();
			}
			
			int første = burgerKo.removeFirst();
			
			System.out.println(Thread.currentThread().getName() + " tar av hamburger (" + første + "). Brett: " + burgerKo);
